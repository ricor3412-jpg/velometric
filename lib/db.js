...
export const deleteScan = async (scanId) => {
  // Cascading deletes should be handled by DB, but we can do it explicitly if needed.
  // In Supabase/Postgres, if foreign keys are ON DELETE CASCADE, this is simple.
  const { error } = await supabase
    .from('scans')
    .delete()
    .eq('id', scanId);
...
export const clearAllScans = async () => {
  // This will delete all records in the scans table.
  // If cascading is set up, results and logs will also go.
  const { error } = await supabase
    .from('scans')
    .delete()

...
export async function auditUrl(url, deviceType = 'desktop', networkProfile = '4g') {
  ...
  if (process.env.VERCEL || process.env.USE_PSI_API === 'true' || process.env.NODE_ENV === 'production') {
    try {
      return await auditWithPSI(url, deviceType);
    } catch (err) {
      console.error("[Auditor] PSI API failed, attempting local fallback if possible...", err.message);
      // If we are in local dev, we can continue to local puppeteer

...
export async function crawlWebsite(startUrl, maxPages = 5) {
  // Normalize startUrl to have trailing slash for consistent comparison
  let crawlStart = startUrl;
  if (!crawlStart.endsWith('/')) crawlStart += '/';
  ...
  // Strategy 1: Sitemap
  let sitemapUrls = await discoverFromSitemap(crawlStart, maxPages);
  
  // Strategy 2: Navigation-based discovery
  let navUrls = [];
  if (sitemapUrls.length < 2) { // Use nav if sitemap is empty or very small
    navUrls = await discoverFromNavigation(crawlStart, maxPages);
  }

  // Normalize all discovered URLs to compare correctly
  const normalize = (u) => u.endsWith('/') ? u : u + '/';
  const startNorm = normalize(crawlStart);

  // Combine everything
  const rawUrls = [crawlStart, ...sitemapUrls, ...navUrls];
  
  // Deduplicate using normalized versions
  const uniqueUrls = [];
  const seen = new Set();
  ...
  // Ensure home is at the very beginning
  const home = uniqueUrls.filter(u => normalize(u) === startNorm);
  const others = uniqueUrls.filter(u => normalize(u) !== startNorm)
    .sort((a, b) => getPagePriority(a) - getPagePriority(b));

  const result = [...home, ...others].slice(0, maxPages);

  console.log(`[Crawler] ✓ Final: ${result.length} pages to audit (Home is ${result[0] === crawlStart ? 'first' : 'at index ' + result.indexOf(crawlStart)})`);
  return result;
}
